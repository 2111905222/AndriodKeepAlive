...
function addtrguidein(event) {
    ...
    if (have_name || (fileInput.value == "0") || (fileInput.value == "1") || (fileInput.value == "2") || (txtInput.indexOf("txt") == -1) || (txtInput == "") || (imgInput == "") || ((imgInput.indexOf("jpg") === -1) && (imgInput.indexOf("png") === -1) && (imgInput.indexOf("jpeg") === -1))) {
        ...
        gen_data_guidein(table, fileInput, fileNum);
        return 0;
    }
...
//// 外部表格输入名称的
function gen_data_guidein(table, fileInput, fileNum) {
    // 创建新的数据行
    // 创建新的一行
    ...
    // 将 tr 数据加入到 data 数组中
    //次序
    if ((fileNum.value >= 1) && (fileNum.value <= data_guidein.length)) {
        data_guidein.splice(fileNum.value - 1, 0, row);
        // console.log(data_guidein);
    }
    else
...
function cancel_button_guideinbj() {
    // 添加事件监听器，当用户点击侧边栏外部时隐藏侧边栏
    // 获取保存和取消按钮元素
    const cancelButton = document.getElementById('cancel-button-guidein-bj');
...
function change_data_guidein(fileInput, fileNum, index) {
    // 创建新的数据行
    // 仅修改了姓名部分
    if (fileNum.value != "") data_guidein[index - 1].children[0].innerText = fileNum.value;
    data_guidein[index - 1].children[1].innerText = fileInput.value;
    var row = data_guidein[index - 1];

    //次序
    if (fileNum.value != "") {
        data_guidein.splice(index - 1, 1); //删除当前行
        if ((fileNum.value >= 1) && (fileNum.value <= data_guidein.length)) {
            data_guidein.splice(fileNum.value - 1, 0, row);//在数组中插入            
        }
        else data_guidein.push(row);
    }
...
//     else {
//         $.ajax({
//             url: "/alterGuideName",//后台请求的数据
//             dataType: 'json', //数据格式
//             type: "POST",//请求方式
//             contentType: "application/json;charset=utf-8",
//             data: JSON.stringify({ guidename: fileInput.value }), //上传的数据
//             success: function (data) {
//             },
//             error: function (arg1) {
//                 alert("上传数据失败");
